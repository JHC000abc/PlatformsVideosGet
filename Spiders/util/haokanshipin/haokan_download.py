...
def get_urllist(user_id):
    ...
    try:
        ...
        while has_more == 1:
            headers = get_headers()
            headers["cookie"] = 'BAIDUID=53ED023F7504FCEA2F8BDFAE0AFFAF24:FG=1; BSG_B_TOKEN=; SECURE_BSG_B_TOKEN=; av1_switch_v3=0; COMMON_LID=279cc2d224aaf133cbbd4512ae0a56d8; PC_TAB_LOG=video_details_page'
...
def get_mp4_urllist(urllist):
    ...
    try:
        for url in urllist:
            headers = get_headers()
            response = requests.get(url=url,headers=headers)
            mp4_url = re.findall("\"previewUrlHttp\":\"(.*?)\",", response.text.encode("utf-8").decode("latin1"))
            if len(mp4_url) != 0:
        ...
        print(e)
        traceback.print_exc()
    print(f"可下载的视频数量为{len(mp4_urllist)}")
    return mp4_urllist
...
def main(args):
    ...
    user_id = args["url_list"][0].split("?")[-1].replace("vid=","")
