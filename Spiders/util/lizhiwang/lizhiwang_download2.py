...
def get_mp4_file(m3u8_list_path,jiemu):
        m3u8_list = os.listdir(os.path.join(m3u8_path,jiemu))
        output_path = f"./output/{jiemu}/"
        for m3u8_filename in m3u8_list:
            ...
            try:
                ...
                if os.path.exists(mp4_filepath):
                    continue
                for i in open(m3u8_path+jiemu+"/"+m3u8_filename,"r"):
...
def main(args):
    save_path = os.path.join(setting.SAVE_PATH, args["platform"], args["kind"], args["author"])
