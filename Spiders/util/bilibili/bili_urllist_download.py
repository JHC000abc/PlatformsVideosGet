...
def get_urllist(space_id):
    ...
    while len(vlist) != 0:
        try:
            ...
            url = "https://api.bilibili.com/x/space/wbi/arc/search?mid={space_id}&ps=30&pn={pn}&order=pubdate".format(space_id=space_id, pn=pn)
...
def main02():
    ...
    for i in open(input_path, "r"):
        ...
        try:
            print(space_id)
            id_list = get_urllist(space_id)
            for id_info in id_list:
                out_file.write(up_name + '\t' + 'https://www.bilibili.com/video/' + id_info['bvid'] + '\t' + id_info['aid'] + '\n')
        except Exception as e:
            print (e)
            traceback.print_exc()
